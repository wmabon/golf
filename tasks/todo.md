...
- [x] Configure ESLint, Prettier, testing framework
- [ ] Set up CI pipeline
- [x] db-architect: Initial schema design aligned to PRD §10 entities

### Phase 1: Identity & Auth (FR-1 to FR-5)
- [x] pm-spec: Extract identity spec → `tasks/specs/identity.md`
- [x] architect-review: Validate identity spec → `tasks/adrs/identity.md`
- [x] db-architect: User, MembershipEntitlement tables + migrations
- [x] identity-agent: Implement auth flow, profile, memberships
- [x] frontend-builder: Auth components, profile UI
- [x] test-writer: Identity acceptance criteria tests

### Phase 2: Trip Creation & Collaboration (FR-6 to FR-10)
- [x] pm-spec: Extract trip spec
- [x] architect-review: Validate trip spec
- [x] db-architect: Trip, TripMember tables + state machine column
- [x] trip-agent: Trip CRUD, invitations, captain role, state transitions
- [x] frontend-builder: Trip home screen, invite flow
- [x] test-writer: Trip acceptance criteria tests

### Phase 3: Course Discovery (FR-11 to FR-22)
- [x] pm-spec: Extract discovery spec
- [x] architect-review: Validate discovery spec
- [x] db-architect: Course, CourseRule, CourseReview, CourseComposite tables + PostGIS indexes
- [x] discovery-agent: Search, filters, quality model, access filtering, recommendations
- [x] frontend-builder: Discovery screen, course cards, course detail page, map/list sync
- [x] test-writer: Discovery acceptance criteria tests

### Phase 4: Voting & Shortlisting (FR-23 to FR-28)
- [x] pm-spec: Extract voting spec
- [x] architect-review: Validate voting spec
- [x] db-architect: TripOption, Vote tables
- [x] trip-agent: Voting logic, shortlist generation, captain override
- [x] frontend-builder: Vote board screen
- [x] test-writer: Voting acceptance criteria tests

### Phase 5: M1 Integration & Polish
- [x] End-to-end flow: signup → create trip → invite → discover → shortlist → vote → converge
- [x] Seed 101 U.S. courses + 50 airports for pilot testing
- [x] M1 exit criteria validation: "pilot group converges on shortlist without ops help"
...
## Admin Console UI
- [x] Admin shell: layout gate, sidebar nav (slate palette)
- [x] Concierge booking ops: request queue + detail + confirmation forms (FR-76)
- [x] Course curation: list, editor, access classification, booking rules (FR-75)
- [x] Config placeholder page
- [ ] Configuration page: fee schedules, feature flags, thresholds (FR-79)
- [ ] Dashboard: stats overview + system alerts
- [ ] Membership verification queue (FR-77)
- [ ] Content moderation console (FR-78)
...
- [x] Documentation: README, CONTRIBUTING, CLAUDE.md, 8 ADRs, API reference (1552 lines), decision log
