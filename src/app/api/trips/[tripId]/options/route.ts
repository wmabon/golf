...
import { createOptionSchema } from "@/lib/validation/trips";
...
export async function GET(_request: Request, { params }: Params) {
  ...
  const options = await optionService.listOptions(tripId);
...
export async function POST(request: Request, { params }: Params) {
  ...
  const parsed = await parseBody(request, createOptionSchema);
